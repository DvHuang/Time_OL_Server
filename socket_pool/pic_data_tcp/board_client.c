...
void main(int argc, char ** argv)
{
   ...
   int Port;//端口
   unsigned char     DataBuffer[65535];//接受数据缓冲区
   char ip[20]={0};
   FILE* fp;
   FILE* File;
...
;
  ...
  cam_fd=open("/dev/camera",O_RDWR);
    if(cam_fd<0)printf("camera error \n");
  read(cam_fd,buff,8192); 
...
//////////////
   ...
   if(connect(s, (struct sockaddr*) &ServerAddr, sizeof(ServerAddr)) == -1)//连接到服务器
   {
