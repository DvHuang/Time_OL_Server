	...
	if(retval>0){
			...
			if(testname=NULL){
				...
				time_min_start=time_h*3600+time_m*60+time_s;
			      ...
			      if (db_store(db, ptr, namechar+stingname, DB_STORE) != 0) err_quit("db_store error for alpha");
				  }
			else{
					int testfinale;
        ...
        { 
				...
				// err_quit("db_store error for alpha");
				 ...
				 time_min_end=time_min_end+dbint;
					...
					if (db_store(db, ptr,namechar+string, DB_STORE) != 0)
					err_quit("db_store error for alpha");
                    close(new_server_socket_fd); 
                    return(-2); 
        } 
        ...
        { 
				...
				if (db_store(db, ptr, namechar+string, DB_INSERT) != 0)
				err_quit("db_store error for alpha");
                close(new_server_socket_fd); 
                return(-4); 
        } 
