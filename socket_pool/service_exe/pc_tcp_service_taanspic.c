...
{ 
   ...
   setsockopt(server_socket,SOL_SOCKET,SO_REUSEADDR,&opt,sizeof(opt));
    ...
    while (1) //服务器端要一直运行
    {
      ...
      //  length_addr = recv(new_server_socket,buffer,BUFFER_SIZE,0);
      //  if (length_addr < 0)
      //  {
...
//            close(fp);
  ...
  //  }
    // under code was copy to client app
    ...
    bzero(buffer,BUFFER_SIZE);
    int length = 0;
    while( length = recv(new_server_socket,(char *)&buffer,Frame_Size,0))
     {
       int i=0;
      // int n=sizeof(buffer);
       for( i =0;i<1;i++){
     ...
     int write_length = fwrite(buffer,sizeof(int),length,fp);
