...
int socket_branch (void){
	...
	*/
	...
	flag = 1;
	ret = setsockopt( server_socket_fd, IPPROTO_TCP, TCP_NODELAY, (char *)&flag, sizeof(flag) );
	if (ret == -1) {
 	 printf("Couldn't setsockopt(TCP_NODELAY)\n");
 	 exit(-1);
	}
	ret = setsockopt(server_socket_fd,SOL_SOCKET,SO_REUSEADDR,(char *)&flag, sizeof(flag) );
	if (ret == -1) {
